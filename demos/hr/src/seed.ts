...
import { peopleCoModule, HR_PERM } from './module.js';

/**
 * PeopleCo demo world (spec/concept.md §3): one multi-country company
 ...
 * checkpoint emitter (parity with demos/fsm).
 */
export const MODULES = [protocolModule, peopleCoModule];
 ...
 * acknowledgements) — vertical policy that differs from ServiceCo, where the
...
export async function seedDemo(host: SqliteScopeHost, dir: string): Promise<DemoWorld> {
  ...
  for (const t of [world.t1, world.t2]) {
    for (const key of ['protocol', 'peopleco']) {
