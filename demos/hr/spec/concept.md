# Demo Vertical — "PeopleCo" (HR, milestone 2 shape-breaker)
...
> designed*). Where ServiceCo ([demos/fsm](../../fsm/spec/concept.md)) proves the engines
> compose, PeopleCo is the deliberate shape-breaker: a domain with **no ready-made engine**,
...
5. **Protocol reuse generalizes.** Onboarding checklists are `engine-protocol` — the same
   engine CykelService and ServiceCo use — proving reuse across a third, non-field vertical.
6. **The payroll boundary is respected.** Variable pay leaves as an export, never a rebuilt
   payroll — the fakturaunderlag pattern (§7.9: integrate deep-domain moats, don't rebuild).

## 2. Shape: a small multi-country company's HR OS

**PeopleCo** = the HR product skin. It runs the staff-facing surfaces (self-service portal,
...
[Skello](https://www.skello.io/en) that shift-based firms lean on. PeopleCo is HR-record-led
...
| Payroll | **Connector, deferred** | variable-pay export file per pay period (the fakturaunderlag-equivalent) |
...
bike shop forced `engine-protocol` out of ServiceCo).
...
### 5.1 PeopleCo's dual role, and the constraint that makes it work

PeopleCo v1 is deliberately **both**: a standalone, sellable HR product *and* the reference
vertical that seeds `engine-absence` for the operational verticals. These don't conflict —
the reusable unit is the **engine**, not the vertical. PeopleCo is the HR-first *packaging*
(directory, self-service app, dashboard, GTM); the absence/time ledger discipline is what
extracts. ServiceCo and the bike shop wanting vacation + HR-time — a field tech booking leave
...
`facility`/`customer` refs it never dereferences. PeopleCo supplies that ref from
`hr_employees`; ServiceCo supplies it from its own technicians. The **directory — who our
...
already has its own people. Corollary: PeopleCo-the-vertical stays thin (vocabulary, screens,
packaging); anything an operational vertical would also need goes through the engine seam,
never PeopleCo-specific code.
...
through a connector stub. This is ServiceCo's Fortnox-export beat, re-cast for HR, and it
...
1. PeopleCo vertical skeleton: manifest, migrations (`hr_*` tables incl. `hr_projects`),
