<!-- GENERATED by tools/permission-diff.mts — do not edit by hand.
     Regenerate: pnpm lint:permissions
     HUMAN CHECKPOINT (CLAUDE.md): an agent may never self-approve a change to this
     file. Reviewer: §3 is the risk surface — a key gaining a role. -->

# Permission snapshot — @substrat-run/demo-hr

18 keys · 2 modules · 3 roles

## 1. Registry — every key a registered manifest declares

| Key | Description | Declared by |
| --- | --- | --- |
| `absence:approve` | Approve or reject a leave request — approval books the ledger (managers, HR admin) | `@substrat-run/demo-hr` |
| `absence:configure` | Define leave types and grant accruals to employees (HR admin) | `@substrat-run/demo-hr` |
| `absence:read` | Read absence balances, ledger, and requests | `@substrat-run/demo-hr` |
| `absence:request` | Request time off (employees, narrowed to their own record) | `@substrat-run/demo-hr` |
| `employee:manage` | Create and read employee records, including salary/national id (HR admin) | `@substrat-run/demo-hr` |
| `expense:approve` | Approve or reject an expense (managers, HR admin) | `@substrat-run/demo-hr` |
| `expense:read` | Read expenses | `@substrat-run/demo-hr` |
| `expense:submit` | Submit an expense (employees, narrowed to their own record) | `@substrat-run/demo-hr` |
| `payroll:export` | Generate the variable-pay export and mark expenses exported (payroll operator) | `@substrat-run/demo-hr` |
| `project:manage` | Manage the projects time books against (HR admin) | `@substrat-run/demo-hr` |
| `protocol:countersign` | Counter-sign an already-signed protocol — a second signature on the same frozen content (customer at pickup) | `@substrat-run/engine-protocol` |
| `protocol:create` | Define protocol templates and start protocol instances on entities | `@substrat-run/engine-protocol` |
| `protocol:fill` | Record responses on an open protocol (append-only) | `@substrat-run/engine-protocol` |
| `protocol:read` | Read protocol templates, instances, responses and signatures | `@substrat-run/engine-protocol` |
| `protocol:sign` | Sign a protocol — freezes it forever (separate from fill: the technician fills, the arbetsledare signs) | `@substrat-run/engine-protocol` |
| `protocol:void` | Void (supersede) a protocol — never deletes | `@substrat-run/engine-protocol` |
| `time:read` | Read time entries and utilization | `@substrat-run/demo-hr` |
| `time:report` | Log worked hours to a project (employees, narrowed to their own record) | `@substrat-run/demo-hr` |

## 2. Roles — as defined by this vertical's provisioning code

Identical in every tenant. Per-tenant customisation is a runtime concern.

| Role | Permissions |
| --- | --- |
| `hr-admin` | `absence:approve`, `absence:configure`, `absence:read`, `employee:manage`, `expense:approve`, `expense:read`, `payroll:export`, `project:manage`, `protocol:create`, `protocol:fill`, `protocol:read`, `protocol:sign`, `protocol:void`, `time:read` |
| `manager` | `absence:approve`, `absence:read`, `expense:approve`, `expense:read`, `protocol:read`, `time:read` |
| `payroll` | `expense:read`, `payroll:export` |

## 3. Coverage — which roles hold each key

| Key | Held by |
| --- | --- |
| `absence:approve` | `hr-admin`, `manager` |
| `absence:configure` | `hr-admin` |
| `absence:read` | `hr-admin`, `manager` |
| `absence:request` | — no role — |
| `employee:manage` | `hr-admin` |
| `expense:approve` | `hr-admin`, `manager` |
| `expense:read` | `hr-admin`, `manager`, `payroll` |
| `expense:submit` | — no role — |
| `payroll:export` | `hr-admin`, `payroll` |
| `project:manage` | `hr-admin` |
| `protocol:countersign` | — no role — |
| `protocol:create` | `hr-admin` |
| `protocol:fill` | `hr-admin` |
| `protocol:read` | `hr-admin`, `manager` |
| `protocol:sign` | `hr-admin` |
| `protocol:void` | `hr-admin` |
| `time:read` | `hr-admin`, `manager` |
| `time:report` | — no role — |

## 4. Entity-narrowed grant shapes

Reachable WITHOUT a role, narrowed to one entity per principal. A key held by
no role in §3 but listed here is deliberate, not a gap.

| Entity type | Permissions granted per entity |
| --- | --- |
| `employee` | `absence:read`, `absence:request`, `expense:read`, `expense:submit`, `protocol:fill`, `protocol:read`, `protocol:sign`, `time:read`, `time:report` |

## 5. Not covered by this artifact

- **The grants themselves** — per-principal, per-entity, minted at runtime with
  random ULIDs. Only their shapes above are representable deterministically.
- **Operator-defined roles** — roles created against a live deployment. This
  artifact covers provisioning code only.
