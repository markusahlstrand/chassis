...
/**
 * Better Auth as the Meridian template's authentication adapter (D-16: identity is a swappable
...
export function buildAuthNode(dir: string, baseURL: string, trustedOrigins: string[]) {
  ...
  const options: BetterAuthOptions = {
    ...
    secret: process.env.BETTER_AUTH_SECRET ?? 'dev-only-secret-substrat-meridian-demo-32c',
    baseURL,
    trustedOrigins,
  };
