...
/**
 * Egenkontroll v0 fill/sign slice (spec views.md §1.2; engine-protocol.md
 * milestone A). Everything here calls serviceco/* protocol operations — the
...
export function ProtocolPanel({ order }: { order: WorkOrder }) {
  ...
  // ServiceCo policy mirrored for the hint only — the block itself is
  // enforced server-side in serviceco/complete-workorder (the guard).
  const requiredKeys = order.kind === 'montage' ? ['egenkontroll-el'] : [];
