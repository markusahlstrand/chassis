...
**v1 demo** = "ServiceCo": a small installations/service firm (el/VVS-flavored).
**v2 skin** = "CykelService": a bike workshop running the *same engines* with different
...
### 5.2 `engine-invoicing` (fakturaunderlag)
...
1. **(3 min) Agent scaffold**: Claude Code, given manifests + contracts + the ServiceCo
   reference, scaffolds the CykelService vertical — schema, permissions, screens — up to
...
2. **(4 min) Business flow**: create order at ElMontage Stockholm → assign technician →
   report time/material → sign protocol (stretch) → complete → fakturaunderlag line
...
3. ServiceCo vertical (forces the SDK surface, zod-openapi HTTP layer, screens).
4. Demo harness: seed script, the attack script, view-as.
5. CykelService skin (agent-scaffolded — this *is* the acceptance test, §5.6).
...
An agent, pointed at the repo, scaffolds CykelService to the checkpoints without human
