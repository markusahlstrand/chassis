...
/**
 * The ServiceCo (fsm) HTTP API — one route table, adapter- and auth-agnostic.
 *
...
export function mountApi(app: Hono<any, any, any>, resolveStub: ResolveStub): void {
  ...
  app.get('/api/customers', async (c) => c.json(await (await S(c)).invoke('serviceco/list-customers')));
  app.post('/api/customers', async (c) =>
    c.json(await (await S(c)).invoke('serviceco/create-customer', await c.req.json())),
  );
  app.post('/api/customers/:id/facilities', async (c) =>
    c.json(
      await (await S(c)).invoke('serviceco/create-facility', {
  ...
  // -- price list ------------------------------------------------------------
  app.get('/api/prices', async (c) => c.json(await (await S(c)).invoke('serviceco/price-list')));
  app.post('/api/prices', async (c) =>
    c.json(await (await S(c)).invoke('serviceco/upsert-price', await c.req.json())),
  );
  ...
  app.post('/api/workorders', async (c) =>
    c.json(await (await S(c)).invoke('serviceco/create-workorder', await c.req.json())),
  );
  ...
  app.get('/api/workorders/:id/timeline', async (c) =>
    c.json(
      await (await S(c)).invoke('serviceco/timeline', {
  ...
  app.post('/api/workorders/:id/complete', async (c) =>
    c.json(await (await S(c)).invoke('serviceco/complete-workorder', { orderId: c.req.param('id') })),
  );
  ...
  app.post('/api/workorders/:id/protocols', async (c) =>
    c.json(
      await (await S(c)).invoke('serviceco/instantiate-protocol', {
        entityType: 'workorder',
        entityId: c.req.param('id'),
        ...(await body(c)),
      }),
  ...
  app.get('/api/portal/orders', async (c) =>
    c.json(await (await S(c)).invoke('serviceco/portal-orders')),
  );
