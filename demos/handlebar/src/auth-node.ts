...
/**
 * Better Auth as the Handlebar template's authentication adapter (D-16: identity is a swappable
 ...
 * Every app origin belongs in the list, or login silently fails on the one that
 * was left out — a confusing way to find out.
 */
export function buildAuthNode(dir: string, baseURL: string, trustedOrigins: string[]) {
  ...
  const options: BetterAuthOptions = {
    ...
    secret: process.env.BETTER_AUTH_SECRET ?? 'dev-only-secret-substrat-handlebar-demo-32c',
    baseURL,
    trustedOrigins,
  };
