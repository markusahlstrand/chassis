...
```ts
import { money, moneyOf, type Money } from '@substrat-run/contracts';
...
```ts
import { addMoney, mulMoney, addDecimal, mulDecimal, compareDecimal } from '@substrat-run/contracts';
...
`@substrat-run/contracts` and not in each engine's utils file.
