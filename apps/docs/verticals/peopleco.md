...
`demos/hr` — a multi-country **HR vertical**: leave and absence, project time reporting,
expenses, and onboarding, in **one role-adaptive app** that is an employee's self-service
tool and a manager's console at once.

## Overview

PeopleCo is the deliberate **shape-breaker**. Every other demo composes a domain engine;
PeopleCo has *none to compose* — there is no absence engine, no time-tracking engine, no
...
| **Permission surface** | [`PERMISSIONS.md`](https://github.com/substrat-run/substrat/blob/main/demos/hr/PERMISSIONS.md) — 18 keys, 3 roles |
...
themselves (their own acknowledgement), not a supervisor — the same engine as ServiceCo,
...
The variable-pay **payroll export** is the ServiceCo *fakturaunderlag* pattern re-cast:
...
the checked-in [`PERMISSIONS.md`](https://github.com/substrat-run/substrat/blob/main/demos/hr/PERMISSIONS.md),
...
- **Manager (desktop-led)** — an approvals **Inbox** (decline always takes a reason), a team
  ...
  **team** roster.
- **Design system** — the fjord-blue "PeopleCo" palette, a colourblind-safe leave-type
...
```sh
pnpm --filter @substrat-run/demo-hr dev
```
...
```sh
pnpm --filter @substrat-run/demo-hr test
```
