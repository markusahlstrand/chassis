...
/**
 * Better Auth as Dashboard's edge authentication adapter — the concrete provider
...
export function buildAuth(env: AuthEnv, origin?: string) {
  const baseURL = origin ?? env.BASE_URL ?? 'http://localhost:8890';
  ...
  return betterAuth({
    ...
    secret: env.BETTER_AUTH_SECRET ?? 'dev-only-secret-substrat-dashboard-demo-32chars',
    baseURL,
    trustedOrigins,
  });
}
