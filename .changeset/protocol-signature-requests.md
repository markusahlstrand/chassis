...
'@substrat-run/engine-test-kit': patch
---

**engine-protocol: signed documents and asynchronous, non-principal signatures.**
...
- **`protocol_signature_requests`** — the missing noun. One row per party a document was sent
  to. Makes multi-party expressible: an instance reaches `signed` only when *every* requested
