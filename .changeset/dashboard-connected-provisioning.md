...
- **Reaching Callout**: `VERTICAL_CALLOUT` service bindings on `substrat-control-plane` (so `provisionInstance('callout')` forwards to the Callout deployment) and on `substrat-router` (the static-binding dispatch fallback, since WfP is off — D-35). `demos/callout`'s `CONTROL_PLANE_URL` is neutralized (calls go over the service binding; only the `/api` path is used).

Steps 3–4 (router, `*.global.substrat.run` DNS + ACM cert) were already live; this is step 5. Requires a deploy: `substrat-fsm` first, then the control plane + router (service bindings) and the dashboard (`CP_SERVICE_TOKEN` = the control plane's `SERVICE_TOKEN`). Verified in code (8/8 dashboard tests, typecheck, boundary-lint, wrangler dry-runs); the cross-worker flow proves out on that deploy.
