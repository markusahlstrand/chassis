# Chassis Kernel — Design Document
...
flowchart TB
    ...
    V -->|"@chassis/sdk only — lint bans raw fetch/DB"| API
    API --> TR
    API --> S
    API --> QG
    S -->|events| SPINE
...
- Human checkpoint: migrations are the second non-negotiable review gate (§4 of the
  plan). `chassis migrate --dry-run` renders the diff per shape; agents can propose,
  never apply.
...
interface ModuleManifest {
  id: ModuleId;                    // '@chassis/engine-workorder'
...
in that scope: kernel tables (`_chassis_*`), engine tables, vertical tables.

Ownership rules:

1. **Namespaced tables.** Each module prefixes its tables (`workorder_*`); the
   `_chassis_` prefix is reserved. Within a module, FKs and joins are fine and
   encouraged.
2. **Cross-module FKs are forbidden.** Cross-module references use the opaque
   ...
   boundaries.
3. **Migrations are module-owned and module-journaled** (`_chassis_migrations` tracks
...
- `@chassis/shell` — seeded from shadcn-admin as a one-time copy (proven on the auth
...
```
@chassis/sdk               — the narrow typed surface verticals import (the only import)
@chassis/kernel            — contracts + kernel services (runtime-agnostic core)
@chassis/adapter-cloudflare
@chassis/adapter-sqlite
@chassis/contracts             — Zod contract schemas (source of truth, D-22) + emitted
                             OAS/JSON Schema artifacts, checked in and CI-diffed
@chassis/cli               — `chassis dev` (pure-SQLite composition), migrate --dry-run,
                             scaffold; the MCP server ships here
@chassis/contract-tests    — the suite every adapter and module must pass
...
| self-approve schema/permission changes | migrations + permission definitions only apply via reviewed path; agents get dry-run only | `chassis migrate` without approval token is inert |

## 11. Testing strategy

1. **Contract tests** (`@chassis/contract-tests`): every kernel contract, run against
...
event envelope + local spine · module manifest loading a toy engine · `chassis dev` +
...
| K-14 | 2026-07-13 | Shared `money` schema in @chassis/contracts: decimal-string amount + ISO 4217 code, branded; engines never invent money representations | Multi-currency evidence (SEK/NOK) + Tier 1/Tier 2 reconciliation needs uniform money handling |
| K-15 | 2026-07-13 | UI composition (§7.4): build-time composition of manifest-declared `ui` contributions into one React app per vertical; entity-view registry for cross-engine rendering; headless-first engine UI with copy-and-own screens; `@chassis/shell` seeded from shadcn-admin; three surfaces (office/field/portal); runtime microfrontends rejected; web-component slot kept as future escape hatch | Federation solves team-scale deployment we don't have and costs theming/versioning/agent ergonomics; the manifest, proof-path checker, and EntityRef registry are exactly the primitives a pluggable UI needs |
| K-16 | 2026-07-13 | In-scope composition (demos/fsm/spec/testrun.md §9.2): engines export plain functions taking `ctx`; a vertical's operation may call them — same transaction, same serialization; registered operations are default bindings of these functions. Plus `ctx.link(child, parent)` writing manifest-declared relation tuples, and kernel-managed at-least-once local event dispatch with a `_chassis_deliveries` journal | Vertical-owned orchestration (D-19) needs one-transaction composition (e.g. price-then-complete); invariants hold because everything still flows through ctx; `link` is the write path for D-23 rule 3 |
